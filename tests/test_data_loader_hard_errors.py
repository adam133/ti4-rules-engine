...
def test_fetch_tech_names_raises_when_data_dir_missing(monkeypatch) -> None:
    ...
    monkeypatch.setattr(_data_loaders, "_TECH_DATA_DIR", pathlib.Path("/tmp/does-not-exist/technologies"))
...
def test_fetch_leader_data_raises_when_data_dir_missing(monkeypatch) -> None:
    ...
    monkeypatch.setattr(_data_loaders, "_LEADERS_DATA_DIR", pathlib.Path("/tmp/does-not-exist/leaders"))
...
def test_load_hyperlane_connections_raises_when_data_file_missing(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        _hyperlanes, "_HYPERLANES_DATA_FILE", pathlib.Path("/tmp/does-not-exist/hyperlanes.properties")
    )
